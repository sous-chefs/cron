...
action_class do
  ...
  def create_template(create_action)
    ...
    file "#{new_resource.cron_name} legacy named cron.d file" do # rubocop:disable Chef/Modernize/CronDFileOrTemplate
      path "/etc/cron.d/#{new_resource.cron_name}"
      action :delete
      only_if { new_resource.cron_name != sanitized_name }
    end

    template "/etc/cron.d/#{sanitized_name}" do # rubocop:disable Chef/Modernize/CronDFileOrTemplate
...
action :delete do
  # cleanup the legacy named job if it exists
  file 'legacy named cron.d file' do # rubocop:disable Chef/Modernize/CronDFileOrTemplate
    path "/etc/cron.d/#{new_resource.cron_name}"
    action :delete
  end

  file "/etc/cron.d/#{sanitized_name}" do # rubocop:disable Chef/Modernize/CronDFileOrTemplate
    action :delete
  end
end
